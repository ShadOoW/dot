...
6. After saving, immediately verify by running:
   Bash: curl -s http://localhost:3111/agentmemory/memories

   Then for each saved lesson, confirm:
   - The content was stored correctly (not truncated or garbled)
   - The wording matches what was approved
   - No duplicates were created

   Present a simple checklist:
   ✅ mem_xxx — "lesson title" — stored correctly
   ⚠️ mem_xxx — "lesson title" — [issue found]

   If any issue is found, fix it without asking — delete the bad entry
   and re-save the correct version.
